...
`Optional` is a good solution as a return type for method that may or may not have a result, so nullable return types are good candidates to be replaced with `Optional`. See also [usage of Java Optional](code-style-and-quality-java.md#java-optional).
